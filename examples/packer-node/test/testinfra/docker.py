...
def test_message_transform_via_default(host):
    cmd = host.run_expect([0], 'docker run cliffano/backpackerexample')
...
def test_message_transform_via_latest_tag(host):
    cmd = host.run_expect([0], 'docker run cliffano/backpackerexample:latest --message "Hello Packer"')
...
#     cmd = host.run_expect([0], f'docker run cliffano/backpackerexample:{version}')
#     assert 'Original: Hello World' in cmd.stdout
